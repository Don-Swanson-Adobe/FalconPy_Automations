...
def update_proposed_sensor_versions_db(event):
    dynamo_resource = boto3.resource("dynamodb")
    sensor_versions_table = dynamo_resource.Table("Proposed_Sensor_Versions")
    body=event["body"]
...
def proposed_sensor_versions_db(version):
    dynamo_resource = boto3.resource("dynamodb")
    sensor_versions_table = dynamo_resource.Table("Proposed_Sensor_Versions")
    print(version)
    message = []
